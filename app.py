...
def teacher_activities(teacher_id, course_id):
    result = db.session.execute('Select teachers.id, courses.id As courses_id, homework.title As homework_title, homework.description As homework_description, homework.student_comment As homework_student_comment, students.name As students_name, students.last_name As students_last_name, homework.limit_date As homework_limit_date, homework.status As homework_status, homework.grade As homework_grade, homework.date_sent As homework_date_sent, homework.file_url As homework_file_url From enrollment Inner Join courses On courses.id = enrollment.course_id Inner Join homework On enrollment.id = homework.enrollment_id Inner Join teachers On teachers.id = courses.teacher_id Inner Join students On students.id = enrollment.student_id Where teachers.id = :val And courses.id = :val2', {'val': teacher_id, 'val2': course_id})
